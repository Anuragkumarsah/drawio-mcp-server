...
import { RenderSubgraphInput, ExportDiagramInput, ModifySubgraphInput, GetDiagramStateInput, } from "./schemas.js";
...
server.tool("render_subgraph", "Render a complete subgraph on the draw.io canvas with auto-sizing and auto-layout. " +
    "Provide nodes and edges; the extension creates shapes, connects them, applies a layout algorithm, " +
    "and optionally returns the result as an SVG image for visual verification.", RenderSubgraphInput.shape, renderSubgraphTool(context));
server.tool("export_diagram", "Export the current draw.io diagram as SVG. " +
    "Use 'svg_image' format to get a base64-encoded image that vision LLMs can render, " +
    "or 'svg_text' to get the raw SVG XML as text for programmatic analysis.", ExportDiagramInput.shape, exportDiagramTool(context));
server.tool("modify_subgraph", "Make incremental changes to the existing diagram: add nodes, remove nodes, " +
    "add edges, remove edges, or update existing node labels/styles. " +
    "Optionally re-run a layout algorithm after changes and return the updated SVG.", ModifySubgraphInput.shape, modifySubgraphTool(context));
server.tool("get_diagram_state", "Retrieve the current state of the draw.io diagram as a JSON object " +
    "listing all nodes (with id, label, style, geometry) and edges (with id, source, target, label, style).", GetDiagramStateInput.shape, getDiagramStateTool(context));
// ── Start ────────────────────────────────────────────────────────────
async function main() {
